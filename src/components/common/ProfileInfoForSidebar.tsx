...
import "./ProfileInfoForSidebar.scss";
import styles from "./ProfileInfoForSidebar.module.scss";
import ProfilePhotoForSidebar from "./ProfilePhotoForSidebar";
...
const ProfileInfo: FC<OwnProps & StateProps> = ({
    ...
    return (
      <div className={styles.topicContainer}>
        ...
        <p className={styles.topicMessagesCounter}>
          {messagesCount
            ? lang("Chat.Title.Topic", messagesCount, "i")
  ...
  function renderPhotoTabs() {
    const totalPhotosLength = Math.max(
      photos.length,
      profilePhotos?.count || 0
    );
    ...
    const activeDashIndex =
      currentPhotoIndex >= MAX_PHOTO_DASH_COUNT
        ? MAX_PHOTO_DASH_COUNT - 1
        : currentPhotoIndex;
        ...
        {enumerator.map((_, i) => (
          <span
            className={buildClassName(
              styles.photoDash,
              i === activeDashIndex && styles.photoDash_current
            )}
          />
        ))}
      </div>
  ...
  return (
    <div
      className={buildClassName("ProfileInfoForSidebar")}
...
export default memo(
  withGlobal<OwnProps>((global, { peerId }): StateProps => {
    const user = selectUser(global, peerId);
    const userStatus = selectUserStatus(global, peerId);
    const chat = selectChat(global, peerId);
    const profilePhotos = selectPeerPhotos(global, peerId);
    const { mediaIndex, chatId: avatarOwnerId } =
      selectTabState(global).mediaViewer;
    const isForum = chat?.isForum;
    const { threadId: currentTopicId } = selectCurrentMessageList(global) || {};
    const topic =
      isForum && currentTopicId
        ? selectTopic(global, peerId, currentTopicId)
        : undefined;

    const emojiStatus = (user || chat)?.emojiStatus;
    const emojiStatusSticker = emojiStatus
      ? global.customEmojis.byId[emojiStatus.documentId]
      : undefined;
